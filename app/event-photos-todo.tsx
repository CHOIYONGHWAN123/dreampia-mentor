...
import { fetchMissingEventPhotoRows } from '@/lib/mentor-todos';
...
export default function EventPhotosTodoScreen() {
  ...
  const load = useCallback(async () => {
    const data = await fetchMissingEventPhotoRows();
    setRows(data);
    setLoading(false);
    setRefreshing(false);
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <FlatList
        ...
        ListHeaderComponent={
          <ThemedText style={styles.headerHint}>강의가 끝난 강의는 사진 2장을 등록해주세요.</ThemedText>
        }
        ListEmptyComponent={
          <ThemedView style={styles.centered}>
            <ThemedText style={styles.emptyText}>등록할 행사사진이 없습니다.</ThemedText>
          </ThemedView>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
